...
function NormalSpeed(pos) {
	this.basePos = this.pos = pos.plus(new Vector(0.3,0.4));
	this.size = new Vector(0.8, 1.0);
	this.wobble = Math.random() * Math.PI * 3;
	

}
NormalSpeed.prototype.type = "normalspeed";
...
NormalSpeed.prototype.act = function(step) {
	this.wobble += step * wobbleSpeed;
	var wobblePos = Math.sin(this.wobble) * wobbleDist;
	this.pos = this.basePos.plus(new Vector(0, wobblePos));
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Original interactive element
  // picking this up will increase the
  // player's speed
...
// Original hazard, if this is picked up
// the players slows down dramatically
...
// Other original interactive element
// in order for the player's normal speed
// to return, this must be picked up
	
  } else if (type == "normalspeed") {
	playerXSpeed = 7;
	this.actors = this.actors.filter(function(other){
	return other != actor;
	...
	// The player must now also collect all speed boosts to win
    if (!this.actors.some(function(actor) {
           return actor.type == "coin" || actor.type == "speedup";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
